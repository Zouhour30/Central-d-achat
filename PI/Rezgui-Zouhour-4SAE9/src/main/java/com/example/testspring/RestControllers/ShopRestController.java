...
public ResponseEntity<FileResponse> uploadFile(
    ...
    try {
        fileName = elBeyService.UploadImage(path, image);
    } catch (IOException e) {
        e.printStackTrace();
        return new ResponseEntity<>(new FileResponse(null, "Image could not be uploaded due to an error on the server!"), HttpStatus.INTERNAL_SERVER_ERROR);
    }
    return new ResponseEntity<>(new FileResponse(fileName, "Image was uploaded successfully!"), HttpStatus.OK);
}
